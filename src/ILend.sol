...
contract iLend {
    ...
    event LendingDone(address indexed borrower, uint256 amount, uint256 timestamp);
    event CollateralDepositDone(
        address indexed depositor, address indexed depositedTo, uint256 amount, uint256 timestamp
    );

    event FundDepoistDone(address indexed depositor, uint256 amount, uint256 timestamp);

    event LiquidationUSDCReceived(
        address indexed liquidator, uint256 usdcAmount, uint256 poolBalance, uint256 timestamp
    );

    event SentETHToLiquidator(address indexed liquidator, uint256 ethAmount, uint256 poolBalance, uint256 timestamp);
    ...
    error LiquidatorUSDCTransferFromFailed(address liquidator, address depositAddress, uint256 amount);

    error LiquidatorUSDCAllowanceTooLowForILend(address liquidator, address depositAddress);

    error LiquidatorDoesNotHaveEnoughUSDC(address liquidator, uint256 requiredAmount, uint256 availableAmount);

    error CollateralPoolDoesNotHaveEnoughETHForLiquidation(
        address collateralAddress, uint256 requiredAmount, uint256 availableAmount
    );
     ...
     */
    function deposit_my_funds(uint256 _amount, uint256 _lockupPeriod) external {
        ...
        emit FundDepoistDone(msg.sender, _amount, block.timestamp);
    }
     ...
     */
    function withdraw_my_deposit(uint256 amount) external {
     ...
     */
    function _deposit_collateral(address _depositor, uint256 _amount) internal {
        /**
         * while deploying turn it to public
         */
        transfer_funds_from_external(
        ...
        emit CollateralDepositDone(_depositor, address(iCollateral), _amount, block.timestamp);
    }

    function deposit_collateral(uint256 _amount) external {
        if (sTesting == true) {
    ...
    function _borrow_usdc(address _borrower) internal returns (uint256) {
        ...
        emit LendingDone(_borrower, borrowAmount, block.timestamp);
        return borrowAmount;
    }

    function deposit_collateral_and_borrow(uint256 _amount) external returns (uint256 borrowAmount) {
        _deposit_collateral(msg.sender, _amount);
        borrowAmount = _borrow_usdc(msg.sender);
    }
     ...
     */
    function get_liquidation_ready_collaterals() external view returns (LiquidationReadyCollateral[][] memory) {
    ...
    function liquidate_position_for_ETH(address _borrower, uint256 _loanID, uint256 _usdcAmount) external {
        ...
        if (token.balanceOf(address(iCollateral)) < ethAmount) {
            revert CollateralPoolDoesNotHaveEnoughETHForLiquidation(
                address(iCollateral), ethAmount, token.balanceOf(address(iCollateral))
            );
        }

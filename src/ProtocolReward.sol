...
contract ProtocolReward {
    ...
    function receive_rewards_to_treasury (address from, uint256 amount) external returns (bool) {
        require(msg.sender == treasuryAddress, "Only treasury can call this function");
        require(usdcContract.transferFrom(from, treasuryAddress, amount), "Transfer failed");
        return true;
    }
    ...
    returns (bool) {
        require(msg.sender == treasuryAddress, "Only treasury can call this function");
        require(usdcContract.balanceOf(treasuryAddress) >= amount, "Insufficient balance in treasury");

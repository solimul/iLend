...
library PriceConverterLib {
    ...
    function get_price(AggregatorV3Interface priceFeed) public view returns (uint256) {
    ...
    function eth_to_USDC(uint256 ethAmount, AggregatorV3Interface priceFeed) public view returns (uint256) {
        uint256 ethPrice = get_price(priceFeed); // ETH price in USD with 18 decimal places
        uint256 ethAmountInUSD = (ethPrice * ethAmount) / 1e18;
        return ethAmountInUSD;
    }

    function eth_to_USD_WEI(uint256 ethAmountWEI, AggregatorV3Interface priceFeed) public view returns (uint256) {
    ...
    function convert_ether_to_WEI(uint256 _amount) public pure returns (uint256) {
    ...
    function convert_USDC_to_WEI(uint256 _amount) public pure returns (uint256) {
    ...
    function convert_WEI_to_ETH(uint256 _amount) public pure returns (uint256) {
        return _amount / ETHER_IN_WEI;
    }

    function convert_WEI_to_USDC(uint256 _amount) public pure returns (uint256) {
        return _amount / USDC_IN_WEI;
    }
}

...
contract LiquidationEngine {
    ...
    event LiquidationReceived(address indexed liquidator, uint256 usdcAmount, uint256 poolBalance, uint256 timestamp);

    error BorrowerDoesNotHaveEnoughUSDCForLiquidation(
        address borrower, uint256 loanID, address liquidator, uint256 usdcSent, uint256 shortfallRequired
    );

    error LiquidatorDoesNotHaveEnoughUSDC(address liquidator, uint256 usdcSent, uint256 usdcBalance);

    error NotEnoughLiquidableETHForDiscountedLiquidation(
        address borrower, uint256 loanID, uint256 availableETH, uint256 requiredETH
    );

    error NotEnoughLiquidableETH(address borrower, uint256 loanID, uint256 availableLiquidableETH, uint256 requiredETH);
     ...
     */
    function quote_liquidation(address _borrower, uint256 _loanID)
        public
        view
    ...
    function update_liquidation_records(
        ...
        if (liquidatorInfo.liquidator == address(0)) {
            // new liquidator
     ...
     */
    function update_on_liquidation_deposit(address _liquidator, address _borrower, uint256 _loanID, uint256 _usdcAmount)
        external
        ...
        returns (uint256)
    {
        ...
        if (ethToTransfer > collateralETH) {
            revert NotEnoughLiquidableETHForDiscountedLiquidation(_borrower, _loanID, collateralETH, ethToTransfer);
        }
     ...
     */
    function is_a_liquidator(address _liquidator) public view returns (bool) {
     ...
     */
    function yet_to_be_liquidated(address _borrower, uint256 _loanID) public view returns (bool) {
        LiquidationReadyCollateral memory col = iLiqReg.get_liquidation_collateral(_borrower, _loanID);
        return col.yetToBeLiquidated;
    }

    function register_caller_contracts(address _iLendContractAddress) external only_owner_contract(msg.sender) {

...
contract Treasury {
    ...
    function withdraw_wrapped_ETH(address payable _to, uint256 _amount) external only_owner_contract(msg.sender) {
    ...
    function withdraw_native_ETH(address payable to, uint256 amount) external only_owner_contract(msg.sender) {
        require(address(this).balance >= amount, "Insufficient native ETH balance");
        to.transfer(amount);
        emit NativeETHWithdrawn(to, amount);
    }

    function update_protocol_reward_records(uint256 _amount, address _from, uint256 _loanID)
        external
        only_payback_contract(msg.sender)
    {
        sProtocolRewardRecords.push(
            ProtocolRewardInfo({amount: _amount, borrowerAddress: _from, loanID: _loanID, dateReceived: block.timestamp})
        );
    }

    function get_wrapped_ETH_balance(address token) external view returns (uint256) {
    ...
    function register_caller_contracts(address _iLendAddress, address _paybackAddress)
        external
    ...
    function update_fees_for_deposit(address _depositor, uint256 _fees) external only_facade_contract(msg.sender) {
        sDepositFees.push(Fee({amount: _fees, provider: _depositor, dateReceived: block.timestamp}));

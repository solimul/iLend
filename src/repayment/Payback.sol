...
contract Payback {
    ...
    error NotAllInterestPaymentSuccessful(
        address borrowerAddress, uint256 loanID, uint256 interestAmount, uint256 principalAmount, uint256 remaining
    );

    error NotAllPrincipalPaymentSuccessful(
        address borrowerAddress, uint256 loanID, uint256 principalAmount, uint256 remaining
    );
    ...
    error BorrowerDoesNotHaveEnoughUSDCForPrincipalRepayment(
        address borrowerAddress, uint256 lentOutAmount, uint256 oldBalance
    );
    ...
    IERC20 private immutable iUSDC;
     ...
     */
    function update_principal_records(address _funderAddress, uint256 _id) internal returns (uint256) {
        ...
        iDeposit.update_principal_payback_record(_funderAddress, _id);
    ...
    function update_interest_record(
        ...
        if (old < lentFromThisDepositAccount) {
            revert BorrowerDoesNotHaveEnoughUSDCForInterestRepayment(
     ...
     */
    function update_protocol_reward_receipt(address _borrowersAddress, uint256 _loanID, uint256 _amount) internal {
     ...
     */
    function update_loan_repayment(address _borrowersAddress, uint256 loanID, uint256 amount)
        external
